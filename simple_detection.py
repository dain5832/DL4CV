...
# load the input image from disk and grab it's dimensions
...
for k in labels.keys():
    ...
    boxes = np.array([x[0] for x in labels[k]])
    proba = np.array([x[1] for x in labels[k]])
    boxes = non_max_suppression(boxes, proba)

    # loop over the bounding boxes again, this time only drawing the ones
    # that were *not* suppressed
    for (xA, yA, xB, yB) in boxes:
        cv2.rectangle(clone, (xA, yA), (xB, yB), (0, 0, 225), 2)
